...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        std::ofstream logFile("../../../benchmarks/viterbi_benchmark_results.txt");
        if (!logFile.is_open())
        {
        ...
        logFile.close();
        std::cout << "A benchmark lefutott! Keresd a 'viterbi_benchmark_results.txt' fajlt a benchmarks mappaban.\n";
